...
- `artifacts/bloom` — the web frontend: Home (`/`), Planner (`/planner`), Subjects (`/subjects`), Subject workspace (`/subjects/:id`), Focus Tracker (`/focus`), Grade Progress (`/progress`)
- `artifacts/api-server/src/routes` — dashboard, planner-items, study-sessions, focus-sessions, progress, quotes, subjects, storage routes
- `lib/db/src/schema` — `plannerItems`, `studySessions`, `userStats`, `subjects`, `subjectNotes`, `subjectFiles`, `focusSessions`, `progressGrades`
...
- **Grade Progress**: `progress_grades` is the single source of truth for the `/progress` table and all term/subject charts. Each row stores one subject with nullable Term 1–4 integer grades; blank terms remain null and are omitted from charts rather than treated as zero. Subjects are supplied from the existing Subjects API, so new subjects become selectable automatically.
...
- Grade Progress (`/progress`): editable central grade database with Subject dropdown, exact Term 1–4 headers, reusable term-performance bar chart, dynamic per-subject line charts, chart tabs, nullable-safe empty states, and live query invalidation after row edits.
