...
- `artifacts/bloom` — the web frontend: Home (`/`), Planner (`/planner`), Subjects (`/subjects`), Subject workspace (`/subjects/:id`)
- `artifacts/api-server/src/routes` — dashboard, planner-items, study-sessions, quotes, subjects, storage routes
- `lib/db/src/schema` — `plannerItems`, `studySessions`, `userStats`, `subjects`, `subjectNotes`, `subjectFiles`
